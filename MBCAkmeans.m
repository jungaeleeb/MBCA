function [newY,delta_,Qstat,pval] = MBCAkmeans(Y,F,batchlabel,targetbatch,depth,kmeansk)
...
%          
%          6) 'kmeansk' is a integer value if one uses k-means clustering to
%          generate F matrix.
%              
%         
...
%% When F matrix is empty: generate F matrix using k-means clustering
   if isempty(F) 
       ...
       if nargin <= 5 || isempty(kmeansk)
       
            kmeansk = min(21, max(floor(p/100),1)); % such that not exceeding 21 cluster
       else
       end
      ...
      % mean centered data: Ymc
       ...
       % Number of factors: q
       % pbyn is reduced to qbyn, centroid of q clusters
       [~, qbyn] = kmeans(Ymc,q);   % <------------------------ kmeans k
       F = qbyn;  
